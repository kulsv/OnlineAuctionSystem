...
$path1= "../videos/".$_FILES['vfile']['name'];
$path2= "../images/".$_FILES['ifile']['name'];
$vfile= $_FILES['vfile']['name'];
if($_FILES['ifile']['name'])
{ 
list($filn, $pass) = explode(".", $_FILES['ifile']['name']);
$spass=strtolower($pass);
$filn=str_replace("-", "_", $filn);
$filn=str_replace(" ", "_", $filn);
$ifile=$filn.".".$spass;
}
else
...
//$ifile= strtolower($_FILES['ifile']['name']);
include("conn.php");
if($vfile!="" && $ifile!="" )
{
$query="update autopost set status='".$status."',image='".$ifile."',path='".$vfile."' where id='".$id."'";
}
if($vfile=="" && $ifile!="" )
{
$query="update autopost set status='".$status."',image='".$ifile."' where id='".$id."'";
}
if($vfile!="" && $ifile=="" )
{
$query="update autopost set status='".$status."',path='".$vfile."' where id='".$id."'";
}
if($vfile=="" && $ifile=="" )
{
$query="update autopost set status='".$status."' where id='".$id."'";
}
mysql_query($query);
...
$filename1=$_FILES['vfile']['name'];

copy($_FILES['vfile']['tmp_name'], $path1);
}
...
$filename2=$_FILES['ifile']['name'];

$handle = new upload($_FILES['ifile']);
  if ($handle->uploaded) {
  
  //list($filn, $pass) = explode(".", $_FILES['image_field']['name']);
  //echo "filename".$filn;
      $handle->file_new_name   = $filename2;
      $handle->image_resize         = true;
      $handle->image_x              = 100;
	  $handle->image_y              = 100;
      $handle->image_ratio_y        = true;
	  $handle->file_overwrite = true;
      $handle->process('../images/thumb/');
      if ($handle->processed) {
          //echo 'image resized';
          //$handle->clean();
      } else {
  ...
  //$handle2 = new upload($_FILES['image_field']);
  //if ($handle2->uploaded) {
  
  //list($filn, $pass) = explode(".", $_FILES['image_field']['name']);
  //echo "filename".$filn;
      $handle->file_new_name   = $_FILES['ifile']['name'];
      $handle->image_resize         = true;
      $handle->image_x              = 300;
	  $handle->image_y              = 300;
      $handle->image_ratio_y        = true;
